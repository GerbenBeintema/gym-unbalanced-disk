...
######################################################################
save_name = 'full_GP_varying_training_points_2'
######################################################################
...
val_prediction = pd.DataFrame(columns=['N_training_points', 'na', 'nb', 'noise_variance', 'kern_variance', 'lengthscale', 'rms_rad', 'rms_deg', 'nrms'])
submission_prediction = pd.DataFrame(columns=['N_training_points', 'na', 'nb', 'noise_variance', 'kern_variance', 'lengthscale', 'rms_rad', 'rms_deg', 'nrms'])
val_simulation = pd.DataFrame(columns=['N_training_points', 'na', 'nb', 'noise_variance', 'kern_variance', 'lengthscale', 'rms_rad', 'rms_deg', 'nrms'])
...
N_training_list = [50000, len(Y_train_all)]
...
for N_training in N_training_list:
    ...
    for set in split_dict.keys():
        ...
        split_dict[set]['results'] = split_dict[set]['results'].append({'N_training_points': N_training,\
