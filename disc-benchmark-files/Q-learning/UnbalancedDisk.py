...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self, umax=3., dt = 0.025):
        ...
        self.reward_fun = lambda self: -1 - np.abs(np.pi-self.th) #example reward function, change this!
        #lambda self: -1 - np.abs(np.pi-self.th)
        self.viewer = None
        self.u = 0 #for visual
        self.reset()
    ...
    def reward_function(self):
        ...
        if self.th > np.pi/2:
            return 1/np.abs((np.pi/2-np.abs(self.th))+0.1) +10
        # if reached top and standing still then large reward
        elif np.abs(self.th) > np.pi-0.1 and np.abs(self.omega) < 1e-2:
            return +1000000
        else:
            return -1 - np.abs(np.pi-np.abs(self.th))*5
